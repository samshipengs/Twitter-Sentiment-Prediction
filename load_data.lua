...
function mnist.loadTrainSet(maxLoad, n_bag, geometry)
   return mnist.loadDataset(mnist.path_trainset, maxLoad, n_bag, geometry)
end

function mnist.loadTestSet(maxLoad, n_bag, geometry)
   return mnist.loadDataset(mnist.path_testset, maxLoad, n_bag, geometry)
end

function mnist.loadDataset(fileName, maxLoad, n_bag)
   mnist.download()
   ...
   if n_bag then
      data = data[{{1,nExample},{},{},{}}]:index(1, n_bag) -- select n_bag columns
      labels = labels[{{1,nExample}}]:index(1, n_bag)
      print('<mnist> done')
   else
