...
class Data(object):
    def __init__(self, file_name, file_path, json_fields = None, csv_fields = None):
	...
	def csv_df(self):
		...
		data_df =  data_df[self.csv_fields]
		data_df.dropna(axis=0, inplace=True) # drop na rows
		return data_df

	# pre-processing text
	def pre_process(self, df):
		print("Note: pre_process chnages the dataframe inplace.")
	...
	def build_wordvec(self, model_name, size = 200):
	...
	def max_len(self, df):
	    ...
	    self.max_length = df['size'].max() 
	    return self.max_length
	# initialize empty arry to fill with vector repsentation
	def convert2vec(self, df, max_total):
